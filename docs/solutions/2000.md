...
两横排数字，连接相同数字，连线不能相交，一个数字只能连一条。返回最多可以连线的数目
...
给一个字符串，返回长度为k的子串中能包含的最多元音字母的数量

### 方法

- 滑动窗口，维护长度为k的窗口，追踪其中的元音字母数量，记录最大值。T: O(n), S: O(1)
...
给一个数值不同的整数数组，去掉最大值和最小值后，计算平均值

### 方法

- 一遍遍历，记录最小值与最大值，记录数值和，最后计算平均值。T: O(N), S: O(1)
...
给一个整数数组和整数target，返回数组非空子序列的数量，其中最小值与最大值的和小于等于target。结果对10^9 + 7取余
...
给一个方阵，返回两条对角线的元素和。不重复计算元素

### 方法

- 遍历方阵，将对角线元素相加，若方阵为奇数，减去中间重复的元素
...
- 无需计算最终乘积，只用统计正负号即可，有0时结果一定为0。还可以统计负数的个数。T: O(n), S: (1)
